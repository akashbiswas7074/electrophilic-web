...
const CrazyDeals = ({ dealsData }: { dealsData: CrazyDealsDataType }) => {
  ...
  const showNavigation = dealsData && dealsData.offers.length > 1;

  // Auto-play logic
  useEffect(() => {
    if (!dealsData || dealsData.offers.length <= 1) return;
    const interval = setInterval(() => {
      setFade(false);
      setTimeout(() => {
        setActiveSlide((prev) => (prev + 1) % dealsData.offers.length);
        setFade(true);
      }, 200);
    }, 5500); // 3.5 seconds per slide
  ...
  const scrollToNext = () => {
    if (dealsData && activeSlide < dealsData.offers.length - 1) {
  ...
  if (!dealsData || !Array.isArray(dealsData.offers) || dealsData.offers.length === 0) {
    return null;
  }

  const deal = dealsData.offers[activeSlide];
  ...
  return (
    <div className="w-full mb-8">
      ...
      <div className="mb-3 px-2 sm:px-4 md:px-6 mx-auto flex flex-col items-center justify-center text-center">
        <h2 className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-bold text-center mt-6 sm:mt-8 mb-2 sm:mb-4 uppercase px-2 sm:px-4">Crazy Deals</h2>
        {/* Navigation arrows can be enabled if needed */}
        {/* {showNavigation && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <button 
              ...
              className="w-8 h-8 flex items-center justify-center border border-gray-300 rounded-full hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed"
              disabled={activeSlide === dealsData.offers.length - 1}
        ...
        >
          <div className="relative w-full rounded-lg overflow-hidden  bg-white">
            <div className="w-full aspect-[4/3] sm:aspect-[16/7] flex items-center justify-center bg-gray-100">
              {deal?.images?.[0]?.url ? (
                <img
                  ...
                  alt={deal?.title || 'Product highlight'}
            ...
            <div className="px-2 sm:px-4 md:px-6 py-3 sm:py-4 text-center">
              <h3 className="font-semibold text-base sm:text-lg mb-1 sm:mb-2">{deal?.title || "Product Name"}</h3>
              {deal?.description && (
                <p className="text-gray-600 text-xs sm:text-sm mb-1 sm:mb-2 line-clamp-2">{deal.description}</p>
              )}
              {deal?.price && (
                <div className="text-primary font-bold text-lg sm:text-xl">₹{deal.price}</div>
              )}
            </div>
          </div>
        </Link>
